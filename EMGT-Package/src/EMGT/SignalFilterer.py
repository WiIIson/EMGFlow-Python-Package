...
# Generates a dictionary of file names and locations from files in a folder
#
# in_path       Root folder to search for files in
# file_ext      Extension of files to read
# expression    Regular expression - if set, will only map files
#               when the name matches the expression
...
# Acceps different file location formats and converts them to
# a formatted file location directory
#
# fileObj       Object to be converted to a file map
# file_ext      Extension of files to read
# expression    Regular expression - if set, will only map files
#               when the name matches the expression
...
#
# Signal            Signal the filters are applied to
# col               Columns of the signal to apply the filters to
...
def NotchFilterSignals(in_path, out_path, sampling_rate, notch, cols=None, expression=None, exp_copy=False, file_ext='csv'):
    ...
    if exp_copy:
        filedirs = ConvertMapFiles(in_path, file_ext=file_ext)
    else:
...
# short_name    If true, uses the the filename as keys in the features
#               dataframe, and if false, uses the filepath. Use this if
...
def AnalyzeSignals(in_bandpass, in_smooth, out_path, sampling_rate, cols=None, expression=None, file_ext='csv', short_name=True):
    ...
    filedirs_b = ConvertMapFiles(in_bandpass, file_ext=file_ext, expression=expression)
    filedirs_s = ConvertMapFiles(in_smooth, file_ext=file_ext, expression=expression)
